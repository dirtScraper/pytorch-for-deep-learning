...
import numpy as np
try:
    # Optional dependency used only for pretty directory printing.
    from directory_tree import DisplayTree  # type: ignore
except ModuleNotFoundError:  # pragma: no cover
    DisplayTree = None

try:
    # Optional dependency used for convenient image plotting.
    from fastai.vision.all import show_image, show_titled_image  # type: ignore
except ModuleNotFoundError:  # pragma: no cover
    show_image = None
    show_titled_image = None
...
def plot_img(img, label=None, info=None, ax=None):
    ...
    if label:
        ...
        title = f"Label: {label}"
        if show_titled_image is not None:
...
def _to_imshow_array(img):
    """
    Convert common image types (PIL / numpy / torch tensor) to an HxWxC numpy array
    suitable for matplotlib's imshow().
    """
    if isinstance(img, np.ndarray):
        arr = img
    else:
        # Lazy torch support without importing torch as a hard dependency.
...
def print_data_folder_structure(root_dir, max_depth=1):
    ...
    # Fallback: print a simple directory tree without external dependencies.
